...
public class WebMvcAutoConfiguration {
	...
	/**
	 * 
	 * <p>1.spring容器启动时配置RequestMappingHandlerMapping实例。
	 * <p>2.RequestMappingHandlerMapping实例在完成属性填充后，调用其afterPropertiesSet方法。
	 * <p>3.afterPropertiesSet调用initHandlerMethods，在initHandlerMethods找出spring容器中含有@Controller和@RequestMapping的实例类并对其进行解析，然后将解析出的信息通过registerHandlerMethod方法用于对其mappingRegistry属性完成成组（urlMapping信息、处理类及处理方法）注册动作
	 * <p>4.在WebMvcConfigurationSupport的requestMappingHandlerMapping完成对RequestMappingHandlerMapping实例的this.interceptors属性的赋值（拦截器的查找）
	 * <p>5.因为RequestMappingHandlerMapping的父类ApplicationObjectSupport实现了ApplicationContextAware接口，所以在RequestMappingHandlerMapping完成属性填充后会调用ApplicationContextAware接口的方法setApplicationContext（详见ApplicationContextAwareProcessor的前置处理方法postProcessBeforeInitialization）
	 * <p>6.在5的setApplicationContext方法中则会完成this.interceptors数组到list的转换
	 * <p>7.DispatcherServlet由DispatcherServletConfiguration进行配置。
	...
	public static class EnableWebMvcConfiguration extends DelegatingWebMvcConfiguration implements ResourceLoaderAware {
		...
		/**
		 *此处配置一个#RequestMappingHandlerMapping实例
		 */
		@Bean
		@Primary
		@Override

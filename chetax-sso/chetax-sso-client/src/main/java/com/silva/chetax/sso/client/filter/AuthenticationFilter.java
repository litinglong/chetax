...
import com.silva.chetax.sso.client.controller.OrderController;
...
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (!StringUtils.isEmpty(token)) {
            ...
            ResponseEntity<String> res = restTemplate.getForEntity("http://127.0.0.1:8080/verifyToken?token=" +
                    token + "&systemUrl=http://127.0.0.1:8081/logout", String.class);
            String sessionId = res.getBody();
            if (!StringUtils.isEmpty(sessionId)) {
                // 说明登录成功，可以与客户端浏览器建立session
                session.setAttribute("isLogin", true);
                logger.info("客户端1与浏览器的sessionId：", session.getId());
        ...
        response.sendRedirect("http://127.0.0.1:8080/toLogin?returnUrl=" + request.getRequestURL());
    }
